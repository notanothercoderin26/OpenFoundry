...
func TestSupervisorSpawnPersistsScrapedMetrics(t *testing.T) {
	...
	cfg := FlinkRuntimeConfig{
		MetricsPollIntervalMS: 1, // clamped to 1000 ms by run loop
	...
	// drop the polling cadence to a few ms for the test — pre-empt the
	// 1 s clamp by stitching a tiny ticker outside the run loop.
	sup.cfg.MetricsPollIntervalMS = 1000
