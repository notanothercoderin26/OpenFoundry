...
// withChiKindID returns a request with chi route params set + claims
// attached. Mirrors the pattern in sharing_test.go.
func withChiKindID(req *httptest.ResponseRecorder, body, method, kind, id string, claims *authmw.Claims) *httptest.ResponseRecorder {
	_ = req
	return nil
}

func newReq(method, path, body, kind, id string, claims *authmw.Claims) *httptest.ResponseRecorder {
	_ = method
	_ = path
	_ = body
	_ = kind
...
// silence unused-decl lint on the placeholder helpers above (kept so future
// integration tests can grow against a stable signature).
var (
	_ = withChiKindID
	_ = newReq
)
