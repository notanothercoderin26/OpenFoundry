...
// jwtDisallowAutogenEnvKeys lists the env vars that switch the loader
// into fail-closed mode: when any is truthy (1/true/yes/on), missing
// secrets are an error instead of being silently regenerated.
// Operators MUST set this in any environment that is not a single-node
// developer box. See doc.go.
var jwtDisallowAutogenEnvKeys = []string{
	"OPENFOUNDRY_JWT_DISALLOW_AUTOGEN",
	"JWT_DISALLOW_AUTOGEN",
}
...
const (
	...
	SecretLoadEmpty
	// SecretLoadAutogenDisabled — the secret file does not exist and
	// OPENFOUNDRY_JWT_DISALLOW_AUTOGEN (or JWT_DISALLOW_AUTOGEN) was
	// truthy, so the loader refused to auto-generate one. This is the
	// expected fail-closed mode for any non-dev deployment: a missing
	// secret on startup must be a hard boot failure, not a silent
	// per-pod regeneration that splits tokens across replicas.
	SecretLoadAutogenDisabled
)
...
func (e *SecretLoadError) Error() string {
	...
	case SecretLoadAutogenDisabled:
		return fmt.Sprintf("JWT secret file at %s does not exist and auto-generation is disabled (OPENFOUNDRY_JWT_DISALLOW_AUTOGEN is set); provide a managed secret via OPENFOUNDRY_JWT_SECRET or mount the file at the configured path", e.Path)
	default:
...
func loadOrGenerateSecret(path string) ([]byte, error) {
	...
	// Fail-closed: when OPENFOUNDRY_JWT_DISALLOW_AUTOGEN is truthy we
	// refuse to invent a signing key. Without this, a container with no
	// persistent volume silently regenerates the secret on every restart
	// and — worse — every replica signs with a different secret, so
	// tokens issued by one pod are rejected by its siblings.
	if autogenDisabled() {
		return nil, &SecretLoadError{Kind: SecretLoadAutogenDisabled, Path: path}
	}
	...
	// Escalated from slog.Warn to slog.Error: silently regenerating a
	// signing secret invalidates every token issued before the restart.
	// Acceptable in dev, never in production. See doc.go for the full
	// failure mode and the auth_secret_autogenerated_total counter.
	slog.Error("generated new JWT signing secret; persisted for unattended restarts. NEVER use this fallback in production — a container without a persistent volume invalidates every token at each restart.", "path", path)
	recordSecretAutogenerated(path)
	return secret, nil
}

// autogenDisabled reports whether any of jwtDisallowAutogenEnvKeys is
// set to a truthy value as parsed by strconv.ParseBool (accepts
// 1/t/T/TRUE/true/True/0/f/F/FALSE/false/False). Unparseable values
// (e.g. "yes", "on") also count as truthy when non-empty — defensive:
// any operator that bothered to set the variable wanted lockdown.
func autogenDisabled() bool {
	for _, key := range jwtDisallowAutogenEnvKeys {
		...
		// Any non-empty unparseable value (e.g. "yes", "on", "lockdown")
		// is treated as truthy: an operator that took the trouble to set
		// the variable did not mean "ignore me".
		return true
	}
	return false
}
