...
func TestLoadOrGenerateFailsClosedWhenAutogenDisabled(t *testing.T) {
	// Not parallel: t.Setenv mutates process env, and we want to
	// observe that the auto-gen counter is NOT incremented.
	dir := filepath.Join(t.TempDir(), "openfoundry-jwt-"+uuid.New().String())
	path := filepath.Join(dir, "jwt.secret")
	t.Setenv("OPENFOUNDRY_JWT_DISALLOW_AUTOGEN", "true")
	...
	var loadErr *authmw.SecretLoadError
	require.True(t, errors.As(err, &loadErr))
	assert.Equal(t, authmw.SecretLoadAutogenDisabled, loadErr.Kind)
	assert.Equal(t, path, loadErr.Path)

	// Neither file nor parent directory may have been touched.
	_, statErr := os.Stat(path)
	assert.True(t, os.IsNotExist(statErr), "secret file must not be created in fail-closed mode")
	_, statErr = os.Stat(dir)
	assert.True(t, os.IsNotExist(statErr), "parent directory must not be created in fail-closed mode")

	after := testutil.ToFloat64(authmw.SecretAutogenCounter().WithLabelValues(path))
	assert.Equal(t, before, after, "auto-gen counter must not increment in fail-closed mode")
}

func TestLoadOrGenerateTreatsExplicitFalseAsAutogenAllowed(t *testing.T) {
	// "false" is explicitly OFF — auto-gen should proceed as normal.
	dir := filepath.Join(t.TempDir(), "openfoundry-jwt-"+uuid.New().String())
	path := filepath.Join(dir, "jwt.secret")
	t.Setenv("OPENFOUNDRY_JWT_DISALLOW_AUTOGEN", "false")
	...
	_, statErr := os.Stat(path)
	require.NoError(t, statErr, "secret file should be created when autogen is allowed")
}
