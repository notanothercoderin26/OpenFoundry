...
func New(
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Route("/workspace", func(wr chi.Router) {
			...
			wr.Get("/saved-searches", ws.ListSavedSearches)
			wr.Post("/saved-searches", ws.CreateSavedSearch)
			wr.Delete("/saved-searches/{id}", ws.DeleteSavedSearch)
			...
			wr.Put("/resources/{kind}/{id}/references", ws.ReplaceResourceReferences)
