...
CREATE TABLE IF NOT EXISTS ontology_project_saved_change_records (
    id UUID PRIMARY KEY,
    project_id UUID NOT NULL REFERENCES ontology_projects(id) ON DELETE CASCADE,
    change_ids JSONB NOT NULL DEFAULT '[]'::jsonb,
    resources JSONB NOT NULL DEFAULT '[]'::jsonb,
    changes JSONB NOT NULL DEFAULT '[]'::jsonb,
    branch_id UUID NULL REFERENCES ontology_project_branches(id) ON DELETE SET NULL,
    proposal_id UUID NULL REFERENCES ontology_project_proposals(id) ON DELETE SET NULL,
    status TEXT NOT NULL DEFAULT 'saved',
    validation_errors JSONB NOT NULL DEFAULT '[]'::jsonb,
    error_details JSONB NOT NULL DEFAULT 'null'::jsonb,
    note TEXT NOT NULL DEFAULT '',
    saved_by UUID NOT NULL,
    saved_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_ontology_project_saved_change_records_project
    ON ontology_project_saved_change_records(project_id, saved_at DESC);
