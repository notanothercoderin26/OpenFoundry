...
| ------------------------------------- | ------------------ | ---------------------------------------------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------ |
| application-composition-service                   | —                  | `of.workflows.*` (`event_bus_control::contracts`)                | Trigger de build/composición (control)                               | **Mantener en NATS**                                         |
...
| `/home/runner/work/OpenFoundry/OpenFoundry/services/event-ingestion-replication-service/src/backends/kafka.rs`            | Stub explícito (PR 2 deferred); no añade dependencia sobre `event-bus-data` |
| `/home/runner/work/OpenFoundry/OpenFoundry/services/event-ingestion-replication-service/src/main.rs:48`                   | Mensaje de log indicando que el backend Kafka está deshabilitado       |
...
1. **Adición silenciosa de `event-bus-data` para semántica de control.**
   ...
   - `pipeline-schedule-service`, `pipeline-build-service`,
     ...
     `application-composition-service`, `notebook-runtime-service`
...
4. **Servicio nuevo con dependencia de bus pero sin entrada.** Cualquier
   servicio nuevo bajo `services/` que dependa de `event-bus-control` o
