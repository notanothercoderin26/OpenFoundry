...
A new service entrypoint is meanwhile created by copying `services/template/` and registering the result in the Helm chart + Argo CD app set + the edge gateway router table (see [`CLAUDE.md`](../../../CLAUDE.md) §"Adding a new service").
...
Until it ships, the `services/template/` boilerplate fills that role for new services, and the `make tools` / `make gen` flow fills it for contracts.
