...
| 6 | ML/AI/apps & retire Rust | 🟡 in progress — `code-repository-review-service` ✅ foundation; `media-transform-runtime-service` ✅ foundation; `entity-resolution-service` ✅ arch+slice; `ontology-exploratory-analysis-service` ✅ substrate-only; `reindex-coordinator-service` ✅ arch+slice; `lineage-service` ✅ HTTP-health mode + Iceberg schema constants (Kafka→Iceberg runtime queued; field IDs pinned for on-disk format compat). Inventory at openfoundry-go/INVENTORY-PHASE6.md. AI-kernel-bound shells blocked on libs/ai-kernel-go (~7515 LOC, multi-iteration). Architecture+slice candidates queued: workflow-automation, federation-product-exchange. pyo3 sidecars (notebook-runtime, pipeline-build, ontology-actions) STOP-and-ask. |
