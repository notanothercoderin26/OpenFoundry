...
service startup via the legacy migration bootstrap. They define the
legacy PostgreSQL tables that backed the Action types feature
...
* The service binary no longer runs the legacy migration bootstrap —
  its only remaining PostgreSQL dependency is the `outbox.events`
  table, whose DDL is owned by `libs/outbox/migrations/`.
...
query site that still relies on these tables is a known gap tracked
under the per-handler S1.4.b follow-ups in the migration plan.
