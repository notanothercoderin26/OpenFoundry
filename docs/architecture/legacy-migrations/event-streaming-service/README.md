# Archived `event-ingestion-replication-service` runtime migrations
...
- The authoritative hot path now lives in the runtime store owned by
  `event-ingestion-replication-service` (`runtime_store.rs`) together with the
...
- The active migration
  `services/event-ingestion-replication-service/migrations/20260502210000_drop_postgres_streaming_runtime.sql`
...
- Runtime store: [`services/event-ingestion-replication-service/src/domain/runtime_store.rs`](../../../services/event-ingestion-replication-service/src/domain/runtime_store.rs)
...
> streaming runtime store, not in `services/event-ingestion-replication-service/migrations/`.
