...
hosted at `services/workflow-automation-service/internal/domain/executor`
(1 297 LOC at the time of archival, 2026-05). It is preserved as
`.legacy` files so the build doesn't attempt to compile them.
...
**Apache Temporal**. The service becomes a thin REST adapter
...
|------------------------------|--------------------------------------------------------------|----------------------------------------------------------------|
| `executor.legacy`            | `services/workflow-automation-service/internal/domain/executor` | Workflows in `workers-go/workflow-automation/workflows/` (S2.3.b). |
...
As of G-S2-PG (2026-05-03), live handlers no longer insert,
...
If a handler in `services/workflow-automation-service/internal/handlers/`
still imports `domain::executor`, it is **orphaned code from before
the archival** (the binary is an empty entry point, so nothing was
being compiled). The handler must be rewritten against the
