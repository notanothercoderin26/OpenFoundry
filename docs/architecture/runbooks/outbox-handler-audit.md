...
- [x] Every **critical active handler** listed in the evidence table below
  ...
  write.
- [x] `event-ingestion-replication-service` publishes `dataset.streaming.changed.v1`
...
| --- | --- | --- |
| `pg-policy` | [`outbox-pg-policy`](../../../infra/helm/infra/manifests/debezium/kafka-connector-outbox-pg-policy.yaml) | `ontology.object.changed.v1`, `ontology.action.applied.v1`, `audit.events.v1`, `lineage.events.v1` |
| `pg-schemas` | [`outbox-pg-schemas`](../../../infra/helm/infra/manifests/debezium/kafka-connector-outbox-pg-schemas.yaml) | `dataset.streaming.changed.v1` |
| `pg-runtime-config` | [`outbox-pg-runtime-config`](../../../infra/helm/infra/manifests/debezium/kafka-connector-outbox-pg-runtime-config.yaml) | `connector.connection.changed.v1` |
...
| event streaming | [`streams::create_stream`](../../../services/event-ingestion-replication-service/src/handlers/streams.rs), [`streams::update_stream`](../../../services/event-ingestion-replication-service/src/handlers/streams.rs) | stream definition create/update | `dataset.streaming.changed.v1` | Handler begins SQL tx, writes `streaming_streams`, seeds schema history in the same tx, then calls [`event_streaming_service::outbox::emit`](../../../services/event-ingestion-replication-service/src/outbox.rs). Contract tests in the same module validate topic/key/schema/idempotent `event_id`. | ✅ |
| event streaming | [`branches::create_branch`](../../../services/event-ingestion-replication-service/src/handlers/branches.rs), [`delete_branch`](../../../services/event-ingestion-replication-service/src/handlers/branches.rs), [`merge_branch`](../../../services/event-ingestion-replication-service/src/handlers/branches.rs), [`archive_branch`](../../../services/event-ingestion-replication-service/src/handlers/branches.rs) | branch lifecycle | `dataset.streaming.changed.v1` | Branch rows mutate inside one SQL tx and enqueue a branch event before commit. Merge/archive payloads pin `target_branch_id`, `merged_sequence_no`, `archived_at`. Tests: [`services/event-ingestion-replication-service/src/outbox.rs`](../../../services/event-ingestion-replication-service/src/outbox.rs). | ✅ |
| event streaming | [`streams::create_window`](../../../services/event-ingestion-replication-service/src/handlers/streams.rs), [`streams::update_window`](../../../services/event-ingestion-replication-service/src/handlers/streams.rs) | window definition create/update | `dataset.streaming.changed.v1` | Window create/update now run inside SQL tx and emit before commit. Tests assert distinct version tokens for created vs updated events and schema-valid payloads. | ✅ |
| event streaming | [`topologies::create_topology`](../../../services/event-ingestion-replication-service/src/handlers/topologies.rs), [`topologies::update_topology`](../../../services/event-ingestion-replication-service/src/handlers/topologies.rs) | topology definition create/update | `dataset.streaming.changed.v1` | Topology handlers validate config, persist, reload row inside tx, and enqueue before commit. Tests validate schema and deterministic keying. | ✅ |
...
- `event-ingestion-replication-service::push_events`
- `event-ingestion-replication-service::replay_dead_letter`
...
- Outbox DDL now exists in:
  ...
  [`services/event-ingestion-replication-service/migrations/20260503010000_outbox.sql`](../../../services/event-ingestion-replication-service/migrations/20260503010000_outbox.sql),
...
- Consolidated cluster bootstrap now provisions shared `outbox` access
  ...
  [`pg-policy-bootstrap-sql.yaml`](../../../infra/helm/infra/manifests/cnpg/clusters/pg-policy-bootstrap-sql.yaml),
  [`pg-schemas-bootstrap-sql.yaml`](../../../infra/helm/infra/manifests/cnpg/clusters/pg-schemas-bootstrap-sql.yaml),
  [`pg-runtime-config-bootstrap-sql.yaml`](../../../infra/helm/infra/manifests/cnpg/clusters/pg-runtime-config-bootstrap-sql.yaml).
- Logical decoding is enabled on:
  [`pg-policy.yaml`](../../../infra/helm/infra/manifests/cnpg/clusters/pg-policy.yaml),
  [`pg-schemas.yaml`](../../../infra/helm/infra/manifests/cnpg/clusters/pg-schemas.yaml),
  [`pg-runtime-config.yaml`](../../../infra/helm/infra/manifests/cnpg/clusters/pg-runtime-config.yaml).
- Topic + ACL + schema registration evidence:
  [`topics-domain-v1.yaml`](../../../infra/helm/infra/manifests/strimzi/topics-domain-v1.yaml),
  [`kafka-user-debezium-connect.yaml`](../../../infra/helm/infra/manifests/debezium/kafka-user-debezium-connect.yaml),
...
4. the chaos test in
   [`infra/helm/infra/manifests/debezium/chaos-test.md`](../../../infra/helm/infra/manifests/debezium/chaos-test.md)
   is signed off.
