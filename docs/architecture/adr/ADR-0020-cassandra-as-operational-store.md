...
- **Supersedes / supplements:**
  - Implicit "Postgres everywhere" pattern visible in
    [infra/helm/infra/manifests/cnpg/clusters/](../../../infra/helm/infra/manifests/cnpg/clusters/) (71
...
- **Related work:** The Cassandra cluster is provisioned by the
  k8ssandra-operator under `infra/helm/infra/manifests/cassandra/` (see
...
- New sub-chart `infra/helm/infra/manifests/cassandra/` based on the **k8ssandra-operator**
  (Apache-2.0).
...
- Implement task S0.2 of the migration plan: bring up the dev
  ...
  `infra/helm/infra/manifests/cassandra/`.
