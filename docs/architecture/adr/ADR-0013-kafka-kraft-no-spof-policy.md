...
- **Related work:**
  - `infra/helm/infra/manifests/strimzi/kafka-cluster.yaml` (Strimzi `Kafka` + `KafkaNodePool` manifest)
  - `tools/kafka-lint/check_kraft.py` (manifest-time contract lint)
  - `infra/helm/infra/observability/prometheus-rules/kafka.yaml` (runtime alerts)
...
`tools/kafka-lint/check_kraft.py` parses `infra/helm/infra/manifests/strimzi/kafka-cluster.yaml` and refuses (exit 1) any manifest where:
...
`infra/helm/infra/observability/prometheus-rules/kafka.yaml` carries two KRaft-contract alerts in addition to the pre-existing operational ones (broker down, under-replicated, ISR shrink, offline partitions, consumer lag):
...
Both rules are unit-tested with `promtool test rules` in `infra/helm/infra/observability/prometheus-rules/tests/kafka_kraft_test.yaml` and validated on every PR by `.github/workflows/prometheus-rules.yml`.
