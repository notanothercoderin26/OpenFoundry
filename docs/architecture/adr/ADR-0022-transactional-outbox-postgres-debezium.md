...
`infra/helm/infra/manifests/cnpg/clusters/pg-policy.yaml` (see
[ADR-0024](./ADR-0024-postgres-consolidation.md)).

## Debezium configuration

Strimzi `KafkaConnect` cluster under `infra/helm/infra/manifests/debezium/`:
...
- Provisioned via Strimzi `KafkaTopic` CRs under
  `infra/helm/infra/manifests/strimzi/topics/`.

## Operational consequences

- New stateful release `infra/helm/infra/manifests/debezium/` (Strimzi
...
- New CI checks:
  - Postgres logical decoding settings present in
    `infra/helm/infra/manifests/cnpg/clusters/pg-policy.yaml`.
