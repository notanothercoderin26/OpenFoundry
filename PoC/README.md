...
Each PoC lives in its own folder. **All five PoCs will be executed on OpenFoundry.** The first three follow the 14-document structure (00 → 13) plus an internal `blockers/` directory cataloging the service-level gaps that prevent end-to-end execution today. The fourth and fifth, [`defence-supply-chain/`](defence-supply-chain/) and [`multidomain-cop/`](multidomain-cop/), are **academic** PoCs whose documentation was originally drafted against a real Palantir Foundry tenancy as a Foundry-native reference design, and therefore follow a different `docs/00 → 15` layout; they will nevertheless be ported and executed on OpenFoundry like the rest — see their READMEs for the rationale.
...
| [`defence-supply-chain/`](defence-supply-chain/) *(academic)* | EU defence supply-chain risk mapping — TED + EDF + OpenSanctions + OpenCorporates | Foundry-native reference design — academic transparency exercise under EU FDI Screening Regulation, to be executed on OpenFoundry | Docs ready, awaiting port to OpenFoundry | [`defence-supply-chain/docs/00_README.md`](defence-supply-chain/docs/00_README.md) |
| [`multidomain-cop/`](multidomain-cop/) *(academic)* | Multi-domain Common Operational Picture for crisis management — IGN seismic + AEMET CAP + NVD CVEs + OpenSky ADS-B + AISStream AIS, fused on one event substrate | Foundry-native reference design — academic resilience-COP exercise; explicitly event-centric, not asset-centric, to be executed on OpenFoundry | Docs ready, awaiting port to OpenFoundry | [`multidomain-cop/docs/00_README.md`](multidomain-cop/docs/00_README.md) |
...
| Academic researcher, transparency NGO, defence-procurement analyst working under EU 2019/452 framing — executed on OpenFoundry | **defence-supply-chain** *(academic — different layout)* |
| Academic researcher, civil-protection coordinator, crisis-management analyst wanting to fuse public open-data event streams (seismic / weather / cyber / maritime / aeronautical) into a single resilience-COP — executed on OpenFoundry | **multidomain-cop** *(academic — different layout)* |
