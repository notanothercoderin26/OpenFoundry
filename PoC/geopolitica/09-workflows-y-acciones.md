...
>
> The subscription rows that actually fan out the side-effects (5 subscriptions across in_app + email + webhook, with SLA + escalation + producer-side marking contract for the Marcos-OFAC UC-7) live in [`assets/notification-routing-geopolitica.yaml`](assets/notification-routing-geopolitica.yaml) and are registered via [`infra/scripts/poc-geopolitica/register-notification-routing.sh`](../../infra/scripts/poc-geopolitica/register-notification-routing.sh) — also smoke-tested at `make test`.
