...
| **G5** Schema-aware mapping registration (consume `ontology.object_type.changed.v1`) | ✅ Done | New `searchabstraction.MappingRegistrar` interface ([libs/search-abstraction/mapping.go](../../libs/search-abstraction/mapping.go)) with neutral `TypeMapping` / `MappingField` shape (string, text, integer, long, double, boolean, date, geo, unknown) — backends opt in via type assertion. New `internal/schemasync` package ([services/ontology-indexer/internal/schemasync/](../../services/ontology-indexer/internal/schemasync/)) owns: the envelope subset (`SchemaEventEnvelope` + `ObjectTypePayload`, deliberately duplicated from the producer's models to keep services decoupled), `MappingFromPayload` (`property_type` → `MappingFieldType` with `base_type`/`type_family` fallbacks), and the `Handler` that dispatches Created/Updated → `RegisterTypeMapping` and Deleted → `DropTypeMapping`. Wired into `runtime.ProcessMessageWithProjector` via `TopicObjectTypeChangedV1` (added to `SubscribeTopics`), and `osv2_projection.go::apply` passes schema topics through to the search side instead of treating them as decode errors. When the backend does not implement `MappingRegistrar` the record is committed as `OutcomeSchemaSkippedNoOp` (debug-logged, never blocks the loop). Verified by `internal/schemasync/translate_test.go` (8 property-type variants), `internal/schemasync/handler_test.go` (created / updated / deleted-with-Before / deleted-via-aggregate_id / no-registrar fallback / registrar error retry / malformed envelope / wrong aggregate), and `runtime_test.go::TestRunWithReaderRoutesObjectTypeEventsToSchemaSync` (Kafka record → MappingRegistrar call). Concrete Vespa application-package upload remains a follow-up — the plumbing is in place; only the backend-side implementation of `MappingRegistrar` on Vespa needs the REST glue. |
