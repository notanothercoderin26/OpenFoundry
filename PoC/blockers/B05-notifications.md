...
| Gap | Status | Evidence |
|---|---|---|
| **AC#1** `POST /subscriptions` persists `{event, channel, target, template}` | ✅ Done | New migration [`0002_subscriptions_events.sql`](../../services/notification-alerting-service/internal/repo/migrations/0002_subscriptions_events.sql) adds `notification_subscriptions` with channel, target, template (JSONB), `hmac_secret`, `sla_seconds`, `escalation_target`, `enabled`. CRUD via `POST/GET/DELETE /api/v1/notifications/subscriptions[/{id}]` ([handlers/subscriptions.go](../../services/notification-alerting-service/internal/handlers/subscriptions.go)). MemoryStore + PgStore mirror; HMAC secret never leaves the server (omitted from wire JSON via `HasHMACSecret bool`). |
| **AC#2** `POST /events` fans out to matching subscriptions | ✅ Done | `Dispatcher.Submit` ([service/fanout.go](../../services/notification-alerting-service/internal/service/fanout.go)) persists the event and creates one `notification_event_deliveries` row per matching enabled subscription. Returns the event + deliveries so the producer sees the fan-out fan-out shape immediately. In-app deliveries are written eagerly into the existing `notifications` inbox so the Approvals UI sees them without waiting for the worker. |
| **AC#3** Webhook HMAC + 3-retry exponential backoff + DLQ | ✅ Done | `Worker.deliverWebhook` signs the body with `HMAC-SHA256(secret, body)` in the `X-OpenFoundry-Signature` header (`sha256=<hex>`) when `hmac_secret` is set. Retry policy: `attempt++`, schedule next attempt at `InitialBackoff * 2^(attempt-1)` clamped to `MaxBackoff`, status flips to `retrying`. After `max_attempts` (default 3), status flips to `failed` and the row is the DLQ — visible via `GET /api/v1/notifications/events/{id}/deliveries`. Uses `FOR UPDATE SKIP LOCKED` so concurrent workers don't double-attempt. |
| **AC#4** Approvals UI consumes real notifications | ✅ Done | New [`ApprovalsPage`](../../apps/web/src/routes/approvals/ApprovalsPage.tsx) at `/approvals` (replaces the previous AuditPage squat) is a two-pane: inbox on the left (real notifications, refetch every 10 s, ✓ Acknowledge button issues `PATCH /notifications/{id}/read`); per-event delivery audit on the right (webhook/in-app/email status, retry count, last_error, escalation state — failed rows ARE the DLQ surface). New API helpers in [`lib/api/notifications.ts`](../../apps/web/src/lib/api/notifications.ts): `listNotificationSubscriptions`, `createNotificationSubscription`, `deleteNotificationSubscription`, `submitNotificationEvent`, `listEventDeliveries`. |
| **AC#5/#6** Action submit / scheduled rule lands in inbox | ✅ Done (producer side) | `libs/ontology-kernel/handlers/actions/execute.go` now calls `emitActionEvent` alongside `emitActionNotifications` on every successful action execute. The new helper POSTs to `/internal/events` with `event_type=action.executed.v1`, the action + actor + target context, and a `title`/`body` ready for the template path. Subscriptions on `action.executed.v1` (in-app, webhook to MRO, …) receive the fan-out without per-action config changes. Workshop-side (B01) wires the same path once unblocked. |
| **SLA escalation** | ✅ Done | `notification_event_deliveries.sla_due_at` is set from `subscription.sla_seconds` at fan-out time. `Worker.SLATick` (15 s default) claims rows whose `sla_due_at` has passed without escalation, marks them `escalated`, and fires `SLAEscalationHook`. The default hook (`LoopbackEscalator`) re-submits the original event on the same dispatcher with `event_type` suffixed `.escalated.v1` so escalation_target subscribers (e.g. manager email) receive it. |
